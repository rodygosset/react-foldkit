...
- Keyed `args` are `Schema.Codec` fields. Omit `toKey` to JSON-encode args (`Schema.toCodecJson`, `Schema.fromJsonString`). Slot key and Interrupt identity share `toKey`.
