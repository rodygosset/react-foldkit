...
- `ReactFoldkit.make` takes `Store.Config` plus `Model: Schema.Codec`. Add `Seed` for pre-activate Model writes (`Schema.toEquivalence` skips equivalent Models). `Provider` is init-only (remove `store` / `fromLive`). `Store.Config.make` constructs a `Store.Config`. `layer` is `NoInfer`'d from `update`, so `Layer.empty` is not accepted when Commands require services.
