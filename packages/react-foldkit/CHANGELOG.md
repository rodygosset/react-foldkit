...
- `CompletedCancelFetch` carries `Query.CancelIntent` (`Replace` or `Forget`). A forget interrupt that lands after re-watch does not start another Fetch. `query.lift(config)` takes a lens or `{ field, parentMessage }` in one call.
