...
describe("interruptible Command.define", function () {
	it("derives the key from args at construction, prefixed by the Command name", function () {
		const RunTask = Command.define("RunTask", {
			...
			execute: function ({ taskId }) {
				return Effect.succeed(SucceededTask({ taskId }))
			},
		})
		...
		const interrupt = RunTask.Interrupt({ taskId: 7 }, function (outcome) {
			return outcome
		})
	...
	it("uses the Command name as the key on the no-args form", function () {
		...
		const interrupt = SyncLibrary.Interrupt(function (outcome) {
			return outcome
		})
	...
	it.effect("interrupts the in-flight holder and reports Interrupted", function () {
		return Effect.gen(function* () {
			...
			const outcome = yield* RunForever.Interrupt(function (outcome) {
				return outcome
	...
	it.effect("reports NotFound when no Command holds the key", function () {
		return Effect.gen(function* () {
			const registry = __makeRegistry()
	...
	it.effect("reports NotFound after the holder completed", function () {
		return Effect.gen(function* () {
			const registry = __makeRegistry()
			...
			const outcome = yield* RunTask.Interrupt({ taskId: 1 }, function (outcome) {
				return outcome
			}).effect.pipe(provideRegistry(registry))
	...
	it.effect("only interrupts the holder of the derived key", function () {
		return Effect.gen(function* () {
			const registry = __makeRegistry()
			...
			const RunTask = Command.define("RunTask", {
				...
				execute: function ({ taskId }) {
					return Effect.onInterrupt(Effect.as(Effect.never, SucceededTask({ taskId })), function () {
						return Effect.sync(function () {
			...
			const outcome = yield* RunTask.Interrupt({ taskId: 2 }, function (outcome) {
				return outcome
			}).effect.pipe(provideRegistry(registry))
	...
	it.effect("same-key invocations run concurrently and Interrupt stops every holder", function () {
		return Effect.gen(function* () {
			const registry = __makeRegistry()
			...
			const Watch = Command.define("Watch", {
				...
				execute: Effect.suspend(function () {
					...
					return Effect.onInterrupt(Effect.as(Effect.never, CompletedWork()), function () {
						return Effect.sync(function () {
			...
			const outcome = yield* Watch.Interrupt(function (outcome) {
				return outcome
			}).effect.pipe(provideRegistry(registry))
	...
	it.effect("releases the key when the Effect fails", function () {
		return Effect.gen(function* () {
			const registry = __makeRegistry()
			...
			const FailingTask = Command.define("FailingTask", {
				messages: [CompletedWork],
				interrupt: true,
				execute: Effect.flatMap(Effect.fail("boom"), function () {
					return Effect.succeed(CompletedWork())
				}),
			})
		...
		function update(model: Model, message: Message): UpdateReturn {
			return Match.value(message).pipe(
				...
				Match.tagsExhaustive({
					Start: function () {
						return [{ status: "running", outcome: null }, [RunForever()]]
					},
					Cancel: function () {
						return [
							model,
							[
								RunForever.Interrupt(function (outcome) {
									return GotOutcome({ tag: outcome._tag })
								}),
							],
						]
					},
					Completed: function () {
						return [{ status: "done", outcome: null }, []]
					},
					GotOutcome: function ({ tag }) {
						return [{ status: "cancelled", outcome: tag }, []]
					},
				})
			)
		}
