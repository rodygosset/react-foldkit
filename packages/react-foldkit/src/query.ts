...
/** A `Context.Service` whose value is `HttpApiClient.Client` and whose `api` is the `HttpApi` used to generate it. */
...
type EndpointSuccessEncoded<Endpoint> = Endpoint extends { readonly "~Success": infer S extends Schema.Constraint }
	? S["Encoded"]
	: unknown
type EndpointError<Endpoint> = Endpoint extends { readonly "~Error": infer S extends Schema.Constraint }
	? S["Type"]
	: unknown
		...
		: Keyed<
				...
				Schema.Codec<
					HashMap.HashMap<
						string,
						{
							readonly args: RequestBody<ClientRequestOf<EndpointFrom<Groups, GroupId, EndpointId>>>
							readonly data: AsyncData.AsyncData<
								EndpointSuccess<EndpointFrom<Groups, GroupId, EndpointId>>,
								EndpointError<EndpointFrom<Groups, GroupId, EndpointId>>
							>
						}
	...
	>(
		...
		Schema.Codec<
			HashMap.HashMap<
				string,
				{
					readonly args: Schema.Schema.Type<Schema.Struct<Fields>>
					readonly data: AsyncData.AsyncData<unknown, unknown>
				}
			>,
			unknown,
			never,
...
function unionCodec(schemas: ReadonlyArray<Schema.Top>): Schema.Top {
	return Array.match(schemas, {
		onEmpty: function () {
			return Schema.Never
		},
		onNonEmpty: function (nonEmpty) {
			return Schema.Union(nonEmpty)
		},
	})
}
...
function errorCodec(endpoint: HttpApiEndpoint.Top): Schema.Top {
	const schemas = new globalThis.Set<Schema.Top>(endpoint.error)
	for (const middleware of endpoint.middlewares) {
		const key = middleware as { readonly error?: Iterable<Schema.Top> }
		if (key.error === undefined) continue
		for (const schema of key.error) {
			schemas.add(schema)
		}
	}
	return unionCodec(globalThis.Array.from(schemas))
}

function payloadCodec(endpoint: HttpApiEndpoint.Top): Schema.Top | undefined {
	const schemas = Array.flatMap(Array.fromIterable(endpoint.payload.values()), function (entry) {
		return entry.schemas
	})
	if (Array.isArrayEmpty(schemas)) return undefined
	return unionCodec(schemas)
}
...
function executeEndpoint(
	client: Context.Service<any, any>,
	group: string,
	endpoint: string,
	request: unknown
): Effect.Effect<unknown, unknown, unknown> {
	return Effect.gen(function* () {
		const apiClient = (yield* client) as globalThis.Record<
			string,
			globalThis.Record<string, (request: unknown) => Effect.Effect<unknown, unknown>>
		>
		const groupClient = Record.get(apiClient, group)
		if (Option.isNone(groupClient)) {
			return yield* Effect.die(new Error(`Query.fromHttpApi: unknown group "${group}"`))
		}
		const method = Record.get(groupClient.value, endpoint)
		if (Option.isNone(method)) {
			return yield* Effect.die(new Error(`Query.fromHttpApi: unknown endpoint "${group}.${endpoint}"`))
		}
...
export function fromHttpApi<Client extends HttpApiService<any, any, any>>(
	...
	return function (config: {
		...
		readonly keyFields?: Array.NonEmptyReadonlyArray<string>
		readonly toKey?: (args: never) => string
	}) {
		...
		if (config.args !== undefined) {
			if (config.toRequest === undefined) {
				throw new Error(`Query.fromHttpApi("${config.name}"): flattened args require toRequest`)
			}
			const toRequest = config.toRequest
			return define({
				name: config.name,
				data,
				error,
				...
				execute: function (args: unknown) {
					return executeEndpoint(client, config.group, config.endpoint, toRequest(args)) as Effect.Effect<
						any,
						any,
						Client
					>
		...
		if (inferred === undefined) {
			return define({
				...
				data,
				error,
				execute: executeEndpoint(client, config.group, config.endpoint, {}) as Effect.Effect<any, any, Client>,
			})
		}
		...
		return define({
			...
			error,
			args: inferred,
			toKey: (config.toKey ?? clientRequestToKey) as never,
			execute: function (args: unknown) {
				return executeEndpoint(client, config.group, config.endpoint, args) as Effect.Effect<any, any, Client>
			},
		} as any)
	} as unknown as FromHttpApi<Client, ApiGroups<Client>>
}
